...
class VideoPlayer(QMainWindow):
    ...
    def play_pause(self):
        state = self.player.state() if self.vlc_available else self.player.state()
        if state == QMediaPlayer.PlayingState:
            self.player.pause() if self.vlc_available else self.player.pause()
            self.play_button.setIcon(self.style().standardIcon(QStyle.SP_MediaPlay))
        else:
            self.player.play() if self.vlc_available else self.player.play()
